...
function CreateRouteModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setName('');
      setDescription('');
      onSuccess(); // This should trigger reload and close
      onClose();   // Explicitly close
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none"
...
function RouteStopListModal({
  ...
  // --- SMART SUGGESTIONS LOGIC (Refined) ---
  const sortedCurrentStops = [...currentRouteStops].sort((a, b) => a.stop_order - b.stop_order);
  const startStop = sortedCurrentStops[0]?.stops;
  const endStop = sortedCurrentStops[sortedCurrentStops.length - 1]?.stops;

  // Use shared hook for consistent, robust suggestions
  // We pass ALL current stops as "intermediate" so they are excluded from suggestions
  const currentStopsList = useMemo(() => currentRouteStops.map(rs => rs.stops), [currentRouteStops]);
  const suggestedStops = useSmartSuggestions(allStops, startStop, endStop, currentStopsList).slice(0, 3);
          ...
          {suggestedStops.length > 0 && (
            <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-lg">
              <h3 className="text-sm font-semibold text-indigo-800 mb-2 flex items-center">
                ...
                Smart Suggestions (Nearby on Route)
              </h3>
              <div className="flex flex-wrap gap-2">
                {suggestedStops.map(stop => (
                  <button
                    ...
                    disabled={loading}
                    className="flex items-center px-3 py-1.5 bg-white border border-indigo-200 text-indigo-700 rounded-full text-xs hover:bg-indigo-100 transition shadow-sm"
                  >
          ...
          <div className="border border-slate-200 p-4 rounded-lg">
            <div className="flex justify-between items-center mb-4">
              ...
              {currentRouteStops.length > 1 && (
                <button
                  ...
                  disabled={loading}
                  className={`text-sm px-3 py-1 rounded-lg transition ${isReordering ? 'bg-amber-600 text-white hover:bg-amber-700' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                    }`}
                >
            ...
            >
              {(isReordering ? reorderedStops : currentRouteStops).map((rs, index) => (
                <div
                  key={rs.id}
                  className={`flex items-center p-3 border border-slate-200 rounded-lg transition ${isReordering ? 'cursor-grab bg-white shadow-md' : 'bg-white'
                    }`}
    ...
    <div className="min-h-screen bg-slate-50">
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto">
                {routes.length === 0 ? (
                  ...
                  routes.map(route => (
                    <div
                      key={route.id}
                      className={`p-4 rounded-lg border transition cursor-pointer ${selectedRoute?.id === route.id
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center mt-2">
                          <span className={`text-xs px-2 py-1 rounded ${route.is_active
            ...
            {selectedRoute ? (
              <div className="space-y-6">
                ...
                <div className={`bg-white rounded-xl shadow-sm p-6 border border-slate-100 transition duration-300 ${isModalOpen ? 'invisible h-0' : 'visible'}`}>
                  ...
                  <Map
                    markers={mapMarkers}
                    // Remove default polyline routeLine in favor of dynamic routing below
                    routes={[]}
                    className="h-[350px]"
                    center={mapCenter}
                  >
                    {/* Inject Routing Control for Path Tracing */}
                    <RoutingControl
      ...
      {showCreateRouteModal && (
        <CreateRouteModal
          ...
          endStopId={endStopId}
          name={newRouteName}
          setName={setNewRouteName}
          description={newRouteDescription}
          setDescription={setNewRouteDescription}
        />
      )}
